...
try {
    ...
    $stmt = $con->prepare("SELECT student_number, student_program, student_college FROM student_info WHERE student_number IN ($placeholders) AND is_active = 1");
    ...
    foreach ($rows as $r) {
        ...
        if ($level == 3 && $userProgram && $r['student_program'] != $userProgram) {
            continue; // skip
        }
        if (($level == 2 || $level == 1) && $userCollege && $r['student_college'] != $userCollege) {
            continue; // skip
        }
        $toDelete[] = $sn;
    }
    ...
    if (count($toDelete) === 0) {
        $con->rollBack();
        echo json_encode(['success'=>false,'message'=>'No students permitted to delete']);
        exit;
    }
    ...
    $updateSql = "UPDATE student_info SET is_active = 0 WHERE student_number IN ($placeholders2)";
    ...
    foreach ($toDelete as $sn) {
        ...
        $auditStmt->execute([$sn, $userId, $audit_reason, 'MASTERLIST']);
    }
