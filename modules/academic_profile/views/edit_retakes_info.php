...
LEFT JOIN
    student_back_subjects AS sb ON si.student_number = sb.student_number
WHERE
    si.is_active = 1
    AND ss.is_active = 1
    ...
    foreach ($studentInfo as $row) {
      ...
      if (!empty($row['general_subject_id'])) {
        $retakes[$row['general_subject_id']] = $row['terms_repeated'];
      }
    }
...
<body>
    ...
    <?php echo renderSidebar(); ?>
        ...
        <div class="content">
          <div class="container-wrapper">
            <div class="container">
              <h2>Student Back Subjects / Retakes Overview</h2>
              <div class="form-container">
                ...
                <div class="form-group full-width">
                  <label>Subjects and Retakes:</label>
                  <div id="subjectGradesList">
                    <?php
                    if (!empty($subjects)) {
                      foreach ($subjects as $subject_id => $subject_name) {
                        $retake = isset($retakes[$subject_id]) ? $retakes[$subject_id] : '';
                        ...
                        echo '<input type="text" value="' . htmlspecialchars($retake) . '" readonly style="width:100px; font-weight:bold;" />';
                        echo '</div>';
                      }
                    } else {
                      echo '<span>No board subjects found for this program.</span>';
                    }
                    ?>
                  </div>
                </div>
                ...
                <div class="button-container"> 
                  <button class="button back-page btn-clear" data-href="back-subjects-retakes" title="Go back to student info.">Back</button>
    ...
    <div id="editStudentModal" class="modal">
        <div class="modal-content">
            <h2>Update Back Subjects / Retakes</h2>
              <form id="addStudentForm" action="modules/academic_profile/processes/submit_academic_profile.php" method="POST">
                ...
                <input type="hidden" name="form_type" value="retakes">
                ...
                <div class="form-group full-width">
                  <label for="retakes">Number of Retakes:</label>
                  <input type="text" name="retakes" id="retakes" required>
                </div>
    ...
    <script>
        ...
        // Wait for genSubOptions to be loaded by the fetch in academic_profile_tables_edit_add.js
        // If genSubOptions is loaded asynchronously, you may need to wait for it
        if (typeof genSubOptions !== "undefined" && Object.keys(genSubOptions).length > 0) {
            populateGeneralSubjects();
        } else {
            // If genSubOptions is not yet loaded, poll until it is
            let tries = 0;
    ...
    <script>
        const retakes = <?php echo json_encode($retakes); ?>;
        $('#subjectCode').on('change', function () {
        const subjectId = $(this).val();
        if (retakes[subjectId]) {
            $('#retakes').val(retakes[subjectId]);
        } else {
            $('#retakes').val('');
        }
    });
