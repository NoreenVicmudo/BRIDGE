...
LEFT JOIN
    student_performance_rating AS sb ON si.student_number = sb.student_number
WHERE
    si.is_active = 1
    AND ss.is_active = 1
    ...
    foreach ($studentInfo as $row) {
      ...
      if (!empty($row['category_id'])) {
        $grades[$row['category_id']] = $row['rating'];
      }
    }
...
");
$ratingStmt->execute(['program_id' => $_SESSION['filter_program']]);
...
<body>
    ...
    <?php echo renderSidebar(); ?>
      ...
      <div class="content">
        <div class="container-wrapper">
          <div class="container">
            <h2>Student Performance Rating Overview</h2>
            <div class="form-container">
              ...
              <div class="form-group full-width">
                  <label>Categories and Ratings:</label>
                  <div id="subjectGradesList">
                    <?php
                    if (!empty($ratings)) {
                      foreach ($ratings as $category_id => $category_name) {
                        $grade = isset($grades[$category_id]) ? $grades[$category_id] : '';
                        echo '<div class="subject-grade-row" style="display:flex;align-items:center;gap:16px;margin-bottom:8px;">';
                        echo '<input type="text" value="' . htmlspecialchars($category_name) . '" readonly style="min-width:200px; margin-right:8px;" />';
              ...
              <div class="button-container"> 
                <button class="button back-page btn-clear" data-href="performance-rating" title="Go back to student info.">Back</button>
  ...
  <div id="editStudentModal" class="modal">
    <div class="modal-content">
        <h2>Update Performance Rating</h2>
        <form id="addStudentForm" action="modules/academic_profile/processes/submit_academic_profile.php" method="POST">
          ...
          <input type="hidden" name="form_type" value="performance_rating">
          <input type="hidden" id="filterProgram" name="filterProgram" value="<?php echo htmlspecialchars($studentData['program'] ?? ''); ?>">
            
          <div class="form-group full-width">
            <label for="ratingCategory">Category:</label>
            <select name="ratingCategory" id="ratingCategory" required>
    ...
    <script>
        ...
        // Wait for categoryOptions to be loaded by the fetch in academic_profile_tables_edit_add.js
        // If categoryOptions is loaded asynchronously, you may need to wait for it
        if (typeof categoryOptions !== "undefined" && Object.keys(categoryOptions).length > 0) {
            populateRatings();
        } else {
            // If categoryOptions is not yet loaded, poll until it is
            let tries = 0;
    ...
    <script>
        const studentGrades = <?php echo json_encode($grades); ?>;
        $('#ratingCategory').on('change', function () {
        const categoryId = $(this).val();
        if (studentGrades[categoryId]) {
            $('#rating').val(studentGrades[categoryId]);
        } else {
            $('#rating').val('');
        }
    });
