...
if (!empty($studentNumbers)) {
    ...
    $gradeStmt = $con->prepare("
        SELECT student_number, simulation_id, student_score, total_score
    ...
    while ($row = $gradeStmt->fetch(PDO::FETCH_ASSOC)) {
        $grades[$row['student_number']][$row['simulation_id']] = $row['student_score'] . '/' . $row['total_score'];
    }
} else {
    exit;
}
...
foreach ($students as $student) {
    ...
    echo "<td><a data-href='edit-simulation-exam-results?studentId=" . htmlspecialchars(urlencode($student['student_id'])) . "' title='Click to edit student info' class='next-page'>" . htmlspecialchars($student['student_number']) . "</a></td>";
    }
    else {
        echo "<td>" . htmlspecialchars($student['student_number']) . "</td>";
    }
    echo "<td>{$full_name}</td>";
    foreach ($simulations as $simulationId => $simulationName) {
        $grade = $grades[$student['student_number']][$simulationId] ?? '-';
        echo "<td>$grade</td>";
    }
    echo "</tr>";
}
