...
if (!empty($studentNumbers)) {
    ...
    $gradeStmt = $con->prepare("
        SELECT student_number, category_id, rating
    ...
    while ($row = $gradeStmt->fetch(PDO::FETCH_ASSOC)) {
        $grades[$row['student_number']][$row['category_id']] = $row['rating'] . '%';
    }
} else {
    exit;
}
...
foreach ($students as $student) {
    ...
    echo "<td><a data-href='edit-performance-rating?studentId=" . htmlspecialchars(urlencode($student['student_id'])) . "' title='Click to edit student info' class='next-page'>" . htmlspecialchars($student['student_number']) . "</a></td>";
    }
    else {
        echo "<td>" . htmlspecialchars($student['student_number']) . "</td>";
    }
    echo "<td>{$full_name}</td>";
    foreach ($ratings as $ratingId => $ratingName) {
        $grade = $grades[$student['student_number']][$ratingId] ?? '-';
        echo "<td>$grade</td>";
    }
    echo "</tr>";
}
