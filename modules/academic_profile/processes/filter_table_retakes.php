...
if (!empty($studentNumbers)) {
    ...
    $retakeStmt = $con->prepare("
        SELECT student_number, general_subject_id, terms_repeated
        FROM student_back_subjects
        WHERE student_number IN ($in)
    ");
    $retakeStmt->execute($studentNumbers);
    $retakes = [];
    while ($row = $retakeStmt->fetch(PDO::FETCH_ASSOC)) {
        $retakes[$row['student_number']][$row['general_subject_id']] = $row['terms_repeated'];
    }
} else {
    exit;
}
...
foreach ($students as $student) {
    ...
    echo "<td><a data-href='edit-back-subjects-retakes?studentId=" . htmlspecialchars(urlencode($student['student_id'])) . "' title='Click to edit student info' class='next-page'>" . htmlspecialchars($student['student_number']) . "</a></td>";
    }
    else {
    ...
    foreach ($subjects as $subjectId => $subjectName) {
        $retake = $retakes[$student['student_number']][$subjectId] ?? 0;
        echo "<td>$retake</td>";
    }
    echo "</tr>";
}
