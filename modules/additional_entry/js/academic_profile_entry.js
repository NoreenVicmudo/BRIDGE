...
function handleMetricChange() {
  ...
  defaultTextbox.style.display = "none";
    ...
    toggleSaveButtonVisibility(false, 'Academic Profile - initial state');

    // Clear all dynamic elements and their labels (program, subject, general subject, rating, simulation)
    [
        "displayProgram",
        "displaySubjects",
        "displayGeneralSubjects",
        "displayRatings",
        "displaySimulations"
    ].forEach(id => {
    ...
    const staticOptions = {
        ...
        "TypeOfSimulation": ["Diagnostic", "Internship", "Practicum", "Add"],
        "Recognition": ["Dean's Lister", "Awards", "Top Performer", "Add"]
    }; */

    if (metric === "BoardSubjects") { // BOARD SUBJECTS BOARD SUBJECTS BOARD SUBJECTS BOARD SUBJECTS BOARD SUBJECTS BOARD SUBJECTS
            ...
            if (selectedCollege && selectedCollege !== "Select") {
                ...
                programSelect.onchange = () => {
                    if (!programSelect.value || programSelect.value === "" || programSelect.value === "Select") {
                        textboxGroup.style.display = "none";
                        defaultTextbox.style.display = "none";
                        toggleSaveButtonVisibility(false, 'Academic Profile - form not ready');
                    }
                    ...
                    if (selectedProgram && selectedProgram !== "Select") {
                        ...
                        populateSubjects(selectedProgram);
                        subjectSelect.appendChild(createOption("AddBoardSubject", "Add Board Subject"));
                        subMetricGroup.appendChild(subjectSelect);

                        // Show additional details and save button only when all dropdowns are selected
                        subjectSelect.onchange = () => {
                            const selectedSubject = subjectSelect.value;
                            const subjectName = (subjectOptions[selectedProgram]?.find(s => s.id == selectedSubject) || {}).name || "";
                            ...
                            if (
                                ...
                                toggleSaveButtonVisibility(true, 'Academic Profile - additional details ready');
                                ...
                                if (selectedSubject !== "AddBoardSubject") {
                                    subjectInput.value = subjectName;
                                    // Check if this subject is hidden and set checkbox accordingly
                                    checkAndSetHideCheckbox('board_subject', selectedSubject);
                                } else {
                                    subjectInput.value = "";
                                }
                                // Set up real-time validation for Board Subjects
                                setupBoardSubjectsValidation();
                            } else {
                                textboxGroup.style.display = "none";
                                defaultTextbox.style.display = "none";
                                toggleSaveButtonVisibility(false, 'Academic Profile - form not ready');
    ...
    else if (metric === "GeneralSubjects") {
        subMetricLabel.textContent = "Select College:";
        populateColleges();

        subMetricSelect.onchange = () => {
            // Always hide additional details and save button if subMetric is not selected
            if (!subMetricSelect.value || subMetricSelect.value === "" || subMetricSelect.value === "Select") {
                textboxGroup.style.display = "none";
                defaultTextbox.style.display = "none";
                toggleSaveButtonVisibility(false, 'Academic Profile - form not ready');
            }
            ...
            // Clear previous subject select and its label
            const oldGenSubSelect = document.getElementById("displayGeneralSubjects");
            if (oldGenSubSelect) {
                const label = oldGenSubSelect.previousElementSibling;
                if (label && label.tagName === 'LABEL') {
                    label.remove();
                }
                oldGenSubSelect.remove();
            }
            ...
            if (selectedCollege && selectedCollege !== "Select") {
                ...
                programSelect.onchange = () => {
                    ...
                    if (selectedProgram && selectedProgram !== "Select") {
                        // Create subject select
                        const genSubSelect = document.createElement("select");
                        genSubSelect.id = "displayGeneralSubjects";
                        // Add subject label
                        const genSubLabel = document.createElement("label");
                        genSubLabel.textContent = "Select Subject:";
                        subMetricGroup.appendChild(genSubLabel);
                        subMetricGroup.appendChild(genSubSelect);

                        populateGeneralSubjects(selectedProgram);
                        genSubSelect.appendChild(createOption("AddSubject", "Add Subject"));
                        subMetricGroup.appendChild(genSubSelect);

                        // Show additional details and save button only when all dropdowns are selected
                        genSubSelect.onchange = () => {
                            const selectedGenSub = genSubSelect.value;
                            const genSubName = (genSubOptions[selectedProgram]?.find(s => s.id == selectedGenSub) || {}).name || "";
                            const genSubInput = document.getElementById("metricTextbox");
                            if (
                                subMetricSelect.value && subMetricSelect.value !== "" && subMetricSelect.value !== "Select" &&
                                programSelect.value && programSelect.value !== "" && programSelect.value !== "Select" &&
                                genSubSelect.value && genSubSelect.value !== "" && genSubSelect.value !== "Select"
                            ) {
                                textboxGroup.style.display = "block";
                                defaultTextbox.style.display = "block";
                                toggleSaveButtonVisibility(true, 'Academic Profile - additional details ready');
                                ...
                                // Pre-fill subject name in textbox if editing
                                if (selectedGenSub !== "AddSubject") {
                                    genSubInput.value = genSubName;
                                    // Check if this subject is hidden and set checkbox accordingly
                                    checkAndSetHideCheckbox('general_subject', selectedGenSub);
                                } else {
                                    genSubInput.value = "";
                                }
                                // Set up real-time validation for General Subjects
                                setupGeneralSubjectsValidation();
                            } else {
                                textboxGroup.style.display = "none";
                                defaultTextbox.style.display = "none";
                                toggleSaveButtonVisibility(false, 'Academic Profile - form not ready');
        ...
        subMetricSelect.onchange = () => {
            ...
            // Clear previous subject select and its label
            const oldRatingSelect = document.getElementById("displayRatings");
            if (oldRatingSelect) {
                const label = oldRatingSelect.previousElementSibling;
                if (label && label.tagName === 'LABEL') {
                    label.remove();
                }
                oldRatingSelect.remove();
            }
            ...
            if (selectedCollege && selectedCollege !== "Select") {
                ...
                programSelect.onchange = () => {
                    ...
                    if (selectedProgram && selectedProgram !== "Select") {
                        ...
                        ratingSelect.id = "displayRatings";
                        // Add subject label
                        const ratingLabel = document.createElement("label");
                        ratingLabel.textContent = "Select Rating:";
                        subMetricGroup.appendChild(ratingLabel);
                        subMetricGroup.appendChild(ratingSelect);

                        populateRatings(selectedProgram);
                        ratingSelect.appendChild(createOption("AddRating", "Add Rating"));
                        subMetricGroup.appendChild(ratingSelect);

                        // Show additional details and save button only when all dropdowns are selected
                        ratingSelect.onchange = () => {
                            const selectedRating = ratingSelect.value;
                            const ratingName = (categoryOptions[selectedProgram]?.find(s => s.id == selectedRating) || {}).name || "";
                            const ratingInput = document.getElementById("metricTextbox");
                            if (
                                subMetricSelect.value && subMetricSelect.value !== "" && subMetricSelect.value !== "Select" &&
                                programSelect.value && programSelect.value !== "" && programSelect.value !== "Select" &&
                                ratingSelect.value && ratingSelect.value !== "" && ratingSelect.value !== "Select"
                            ) {
                                textboxGroup.style.display = "block";
                                defaultTextbox.style.display = "block";
                                toggleSaveButtonVisibility(true, 'Academic Profile - additional details ready');
                                ...
                                // Pre-fill subject name in textbox if editing
                                if (selectedRating !== "AddRating") {
                                    ratingInput.value = ratingName;
                                    // Check if this rating is hidden and set checkbox accordingly
                                    checkAndSetHideCheckbox('rating_category', selectedRating);
                                } else {
                                    ratingInput.value = "";
                                }
                                // Set up real-time validation for Rating Categories
    ...
    else if (metric === "TypeOfSimulation") {
        subMetricLabel.textContent = "Select College:";
        populateColleges();
        ...
        subMetricSelect.onchange = () => {
            ...
            // Clear previous subject select and its label
            const oldSimulationSelect = document.getElementById("displaySimulations");
            if (oldSimulationSelect) {
                const label = oldSimulationSelect.previousElementSibling;
                if (label && label.tagName === 'LABEL') {
                    label.remove();
                }
                oldSimulationSelect.remove();
            }
            ...
            if (selectedCollege && selectedCollege !== "Select") {
                ...
                programSelect.onchange = () => {
                    ...
                    if (selectedProgram && selectedProgram !== "Select") {
                        ...
                        simulationSelect.id = "displaySimulations";
                        // Add subject label
                        const simulationLabel = document.createElement("label");
                        simulationLabel.textContent = "Select Simulation:";
                        subMetricGroup.appendChild(simulationLabel);
                        subMetricGroup.appendChild(simulationSelect);

                        populateSimulations(selectedProgram);
                        simulationSelect.appendChild(createOption("AddSimulation", "Add Simulation"));
                        subMetricGroup.appendChild(simulationSelect);

                        // Show additional details and save button only when all dropdowns are selected
                        simulationSelect.onchange = () => {
                            const selectedSimulation = simulationSelect.value;
                            const simulationName = (simulationOptions[selectedProgram]?.find(s => s.id == selectedSimulation) || {}).name || "";
                            const simulationInput = document.getElementById("metricTextbox");
                            if (
                                subMetricSelect.value && subMetricSelect.value !== "" && subMetricSelect.value !== "Select" &&
                                programSelect.value && programSelect.value !== "" && programSelect.value !== "Select" &&
                                simulationSelect.value && simulationSelect.value !== "" && simulationSelect.value !== "Select"
                            ) {
                                textboxGroup.style.display = "block";
                                defaultTextbox.style.display = "block";
                                toggleSaveButtonVisibility(true, 'Academic Profile - additional details ready');
                                ...
                                // Pre-fill subject name in textbox if editing
                                if (selectedSimulation !== "AddSimulation") {
                                    simulationInput.value = simulationName;
                                    // Check if this simulation is hidden and set checkbox accordingly
                                    checkAndSetHideCheckbox('simulation_exam', selectedSimulation);
                                } else {
                                    simulationInput.value = "";
                                }
                                // Set up real-time validation for Simulation Exams
                                setupSimulationValidation();
                            } else {
                                textboxGroup.style.display = "none";
                                defaultTextbox.style.display = "none";
                                toggleSaveButtonVisibility(false, 'Academic Profile - form not ready');
...
function saveDataEntry() {
    ...
    if (metric === "BoardSubjects") { // WORKING WORKING WORKING
    ...
    // Additional Entry: GENERAL SUBJECTS
    // Sends the selected Program, the chosen General Subject (or Add), and the input name
    // to the PHP process for insert/update. Mirrors the Board Subjects logic.
    else if (metric === "GeneralSubjects") {
        data.append("metric", metric);
        data.append("program_id", document.getElementById("displayProgram")?.value || "");
        data.append("general_subject_id", document.getElementById("displayGeneralSubjects")?.value || "");
        data.append("general_subject_name", document.getElementById("metricTextbox")?.value || "");
        // Handle hide functionality
        const hideCheckbox = document.getElementById("hideFieldCheckbox");
        if (hideCheckbox) {
            data.append("is_hidden", hideCheckbox.checked ? "1" : "0");
    ...
    // Additional Entry: TYPE OF RATING
    // Sends the selected Program, the chosen Rating Category (or Add), and the input name
    // to the PHP process for insert/update. Follows the same pattern as Board Subjects.
    else if (metric === "TypeOfRating") {
        data.append("metric", metric);
        data.append("program_id", document.getElementById("displayProgram")?.value || "");
        data.append("category_id", document.getElementById("displayRatings")?.value || "");
        data.append("category_name", document.getElementById("metricTextbox")?.value || "");
        // Handle hide functionality
        const hideCheckbox = document.getElementById("hideFieldCheckbox");
        if (hideCheckbox) {
            data.append("is_hidden", hideCheckbox.checked ? "1" : "0");
        ...
        data.append("metric", metric);
        data.append("program_id", document.getElementById("displayProgram")?.value || "");
        data.append("subject_id", document.getElementById("displaySimulations")?.value || "");
        data.append("subject_name", document.getElementById("metricTextbox")?.value || "");
        // Handle hide functionality
        const hideCheckbox = document.getElementById("hideFieldCheckbox");
        if (hideCheckbox) {
    ...
    fetch("/bridge/modules/additional_entry/processes/submit_academic_profile_entry.php", {
        method: "POST",
        body: data
    })
    .then(res => res.text())
...
function deleteDataEntry() {
    if (confirm("Are you sure you want to delete this data?")) {
        showToast("Data has been successfully deleted!"); 
    }
}
...
function populateSubjects(programId) {
...
function populateGeneralSubjects(programId) {
  const genSubSelect = document.getElementById("displayGeneralSubjects");
...
function populateRatings(programId) {
  const ratingSelect = document.getElementById("displayRatings");
...
function populateSimulations(programId) {
  const simulationSelect = document.getElementById("displaySimulations");
  simulationSelect.innerHTML = '<option value="" disabled selected>Select</option>';
...
function hideValidationError(containerId) {
    ...
    // Don't directly manipulate save button here
}

// Board Subjects validation
function setupBoardSubjectsValidation() {
    const subjectInput = document.getElementById("metricTextbox");
    const programSelect = document.getElementById("displayProgram");
    const subjectSelect = document.getElementById("displaySubjects");
    ...
    const validateBoardSubjects = async () => {
        ...
        if (subjectName && programId && subjectId) {
            ...
            const result = await validateDuplicateData('BoardSubjects', data);
            if (!result.isValid) {
                showValidationError('boardSubjectsError', result.message);
            } else {
                hideValidationError('boardSubjectsError');
            }
        } else {
            hideValidationError('boardSubjectsError');
        }
    ...
    // Add event listeners
    subjectInput.addEventListener('input', validateBoardSubjects);
    programSelect.addEventListener('change', validateBoardSubjects);
    subjectSelect.addEventListener('change', validateBoardSubjects);
}

// General Subjects validation
function setupGeneralSubjectsValidation() {
    const subjectInput = document.getElementById("metricTextbox");
    const programSelect = document.getElementById("displayProgram");
    const subjectSelect = document.getElementById("displayGeneralSubjects");
    
    if (!subjectInput || !programSelect || !subjectSelect) return;

    const validateGeneralSubjects = async () => {
        const subjectName = subjectInput.value.trim();
        const programId = programSelect.value;
        const subjectId = subjectSelect.value;
        ...
        if (subjectName && programId && subjectId) {
            ...
            const result = await validateDuplicateData('GeneralSubjects', data);
            if (!result.isValid) {
                showValidationError('generalSubjectsError', result.message);
            } else {
                hideValidationError('generalSubjectsError');
            }
        } else {
            hideValidationError('generalSubjectsError');
        }
    ...
    // Add event listeners
    subjectInput.addEventListener('input', validateGeneralSubjects);
    programSelect.addEventListener('change', validateGeneralSubjects);
    subjectSelect.addEventListener('change', validateGeneralSubjects);
}

// Rating Categories validation
function setupRatingValidation() {
    const ratingInput = document.getElementById("metricTextbox");
    const programSelect = document.getElementById("displayProgram");
    const ratingSelect = document.getElementById("displayRatings");
    
    if (!ratingInput || !programSelect || !ratingSelect) return;

    const validateRating = async () => {
        const categoryName = ratingInput.value.trim();
        const programId = programSelect.value;
        const categoryId = ratingSelect.value;
        
        if (categoryName && programId && categoryId) {
            const data = {
                program_id: programId,
            ...
            const result = await validateDuplicateData('TypeOfRating', data);
            if (!result.isValid) {
                showValidationError('ratingError', result.message);
            } else {
                hideValidationError('ratingError');
            }
        } else {
            hideValidationError('ratingError');
        }
    ...
    programSelect.addEventListener('change', validateRating);
    ratingSelect.addEventListener('change', validateRating);
}

// Simulation Exams validation
function setupSimulationValidation() {
    const simulationInput = document.getElementById("metricTextbox");
    const programSelect = document.getElementById("displayProgram");
    const simulationSelect = document.getElementById("displaySimulations");
    
    if (!simulationInput || !programSelect || !simulationSelect) return;

    const validateSimulation = async () => {
        const simulationName = simulationInput.value.trim();
        const programId = programSelect.value;
        const simulationId = simulationSelect.value;
        
        if (simulationName && programId && simulationId) {
            const data = {
                program_id: programId,
            ...
            const result = await validateDuplicateData('TypeOfSimulation', data);
            if (!result.isValid) {
                showValidationError('simulationError', result.message);
            } else {
                hideValidationError('simulationError');
            }
        } else {
            hideValidationError('simulationError');
        }
    ...
    // Add event listeners
    simulationInput.addEventListener('input', validateSimulation);
    programSelect.addEventListener('change', validateSimulation);
    simulationSelect.addEventListener('change', validateSimulation);
}
